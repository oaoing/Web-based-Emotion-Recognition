from keras.models import Model  # model 시작 함수
from keras.layers import Dense, Activation, Flatten, Conv2D, MaxPooling2D, Input, BatchNormalization, add  # layer 함수들
...
def core_model(size, version):

    model = None
    n = 3
    if version == 1:
        depth = n * 6 + 2
        model = resnet_v1(size, depth=depth)
    elif version == 2:
        depth = n * 9 + 2
        model = resnet_v2(size, depth=depth)
...
def set_callback(patience):
    #early_stopping = EarlyStopping(monitor='loss', min_delta=0, patience=patience, verbose=0, mode='auto')  # 오차가 증가하면 학습 조기종료
    ...
    return [tensorboard, lr_reducer, lr_scheduler]
...
if __name__ == '__main__':
    ...
    model = core_model(size, version)
    model.fit(x_train, y_train, batch_size, epochs, callbacks=set_callback(patience), shuffle=True, verbose=0)

    model.save('./model_default_resnet_v1.h5')
